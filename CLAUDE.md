...
- **`MuscleMap.jsx` (850+ lines)** — handles upload, image analysis, exercise confirmation, session save, and gym-class linking in a single file. Target split: `UploadStep`, `AnalyzeStep`, `ConfirmStep` sub-components; keep the `useReducer` in the parent.
- **`History.jsx` (810+ lines)** — `MonthGrid`, session list, and the per-session edit panel should be extracted into named sub-components. The per-session edit state (`Map<sessionId, editState>`) is complex enough to warrant its own hook.
